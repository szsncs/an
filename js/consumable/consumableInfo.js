...
var consumable={
	...
	i:0,	
	type : "1",
	viewid : "com.sunnercn.consumable.ConsumableController",
	init:function(){
		consumable.initPage();
		consumable.initInfo();
	},
	...
	commitCallAction : function(json){//调用删除服务
		...
		callAction(consumable.viewid,"commit",json,"deletecallBack");
	},
	unCommit : function(){
		var data = $cache.read("logininfo");
		var logininfo = JSON.parse(data);	
	...
	unCommitCallAction : function(json){//调用删除服务
		...
		callAction(consumable.viewid,"unCommit",json,"deletecallBack");
	},
	update:function(){
		var data = $cache.read("logininfo");
		var logininfo = JSON.parse(data);
	...
	sumbitCallAction : function(json){//调用新增服务
		...
		callAction(consumable.viewid,"save",json,"savecallBack");
	},
	deleteCallAction : function(json){//调用删除服务
		summer.showProgress({
			"title" : "加载中..."
		});
		callAction(consumable.viewid,"delete",json,"deletecallBack");
	},
	updateCallAction : function(json){//调用修改服务
		summer.showProgress({
			"title" : "加载中..."
		});
		callActionContainsFile(consumable.viewid,"update",consumable.image,json,"updatecallBack");
	},
	initInfo : function(){//详情初始化
		...
		callAction(consumable.viewid,"queryInit",json,"queryInitcallBack");
	},
