...
var feedApply = {
	...
	initLoadData:function(pk_total_info_h){
        ...
        callAction(feedApply.viewid,"initSurplusFeedInfo",json,"callBack");
	},
	/**
	 * 事件绑定
	 */
	...
	delete:function(){
		...
		if (pk_total_info_h && pk_total_info_h.length>0){
	        ...
	        callAction(feedApply.viewid,"deleteSurplusFeedTotal",json,"deletecallBack");
		}else{
			alert("数据出错，请重新登录！");
		}
	},
	...
	submit:function(){
		...
		if (pk_total_info_h && pk_total_info_h.length>0){
	        ...
	        callAction(feedApply.viewid,"submitSurplusFeedTotal",json,"submitcallBack");
		}else{
			alert("数据出错，请重新登录！");
		}
	},
	...
	unSubmit:function(){
        ...
        callAction(feedApply.viewid,"unSubmitSurplusFeedTotal",json,"unSubmitcallBack");
	},
	/**
	 *初始化界面
	 */
	...
	update:function(){
		...
		if(array.length > 0){
	        ...
	        callAction(feedApply.viewid,"updateSurplusFeedTotal",json,"updatecallBack");
		}else{
			alert("请先修改申请数量！");
		}
	},
	...
	apply:function(){
		...
		if(array.length > 0){
	        ...
	        callAction(feedApply.viewid,"applySurplusFeedTotal",json,"applycallBack");
		}else{
			alert("数据出错，请刷新后重试");
		}
